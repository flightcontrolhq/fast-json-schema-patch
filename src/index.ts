...
export interface CompiledPatcher {
  createPatch: (doc1: JsonValue, doc2: JsonValue) => Operation[];
}

type WorkStack = [JsonValue, JsonValue, string, /* schema path */ string];

function generateIterativeDiff(
  plan: Plan
): (
  doc1: JsonValue,
  doc2: JsonValue,
  ...
  // Pre-compile the specialized array diffing functions.
  const specializedArrayDiffs = new Map<
    string,
    (
      arr1: JsonArray,
      arr2: JsonArray,
  ...
  for (const [path, arrayPlan] of plan.entries()) {
    switch (arrayPlan.strategy) {
      case "primaryKey":
        specializedArrayDiffs.set(
          path,
          (arr1, arr2, p, pa, ws) =>
            diffArrayByPrimaryKey(
              arr1,
              ...
              p,
              pa,
              (i1, i2, ip) => ws.push([i1, i2, ip, ip.replace(/\/\d+$/, "")]),
      ...
      case "unique":
        specializedArrayDiffs.set(
          path,
          (arr1, arr2, p, pa) => diffArrayUnique(arr1, arr2, p, pa)
        );
        break;
      case "lcs":
        specializedArrayDiffs.set(
  ...
  // This is the main, iterative diff function.
  return (doc1: JsonValue, doc2: JsonValue, patches: Operation[]) => {
    const workStack: WorkStack[] = [[doc1, doc2, "", ""]];

    while (workStack.length > 0) {
      const [obj1, obj2, path, schemaPath] = workStack.pop() as WorkStack;
      ...
      if (Array.isArray(obj1)) {
        ...
        const arrayDiffer = specializedArrayDiffs.get(schemaPath);
        if (arrayDiffer) {
          arrayDiffer(arr1, arr2, path, patches, workStack);
        } else {
          // Fallback for arrays not in the plan (should be rare)
          diffArrayLCS(
            arr1,
            arr2,
            path,
            patches,
            (i1, i2, ip) =>
              workStack.push([i1, i2, ip, ip.replace(/\/\d+$/, "")]),
            plan.get(schemaPath)?.hashFields
          );
        }
        continue;
      }
      ...
      for (const key of keys2) {
        ...
        if (!keys1.has(key)) {
          patches.push({ op: "add", path: newPath, value: o2[key] });
        } else {
          const val1 = o1[key];
          const val2 = o2[key];
          if (val1 !== val2) {
            // This is the only place where we need to handle undefined for the stack
            if (val1 === undefined) {
              patches.push({ op: "add", path: newPath, value: val2 });
            } else if (val2 === undefined) {
              // This case is handled by the removal loop above, but we add it for completeness
              patches.push({ op: "remove", path: newPath });
            } else {
              workStack.push([val1, val2, newPath, newPath]);
            }
          }
        }
      }
...
export function buildCompiledPatcher(plan: Plan): CompiledPatcher {
  const iterativeDiff = generateIterativeDiff(plan);
  return {
    createPatch: (doc1: JsonValue, doc2: JsonValue): Operation[] => {
      const patches: Operation[] = [];
      iterativeDiff(doc1, doc2, patches);
      return patches;
    },
  };
}
