...
// trim (§5.5.0) consumed both positions as "common" and emitted ZERO ops — silent wrong output.
// Fixed (src/performance/deepEqual.ts): an array-vs-object kind check precedes the fast path, so
// [] ≠ {} (§2.4.2), interning distinguishes them, and Myers (§5.5.2) finds the crossing common
// element (the empty containers appear on both sides at swapped indices, LCS length 1), yielding a
// remove+add pair that round-trips exactly (§7.1). NOTE: a remove+add, not two positional replaces —
...
D("kind-mismatch", {
  ...
  comment: "§2.4.1/§2.4.2 (D1): [] ≠ {} — a zero-members fast path that skips the array-vs-object type check is non-conforming; diff {x:[[],{}]}->{x:[{},[]]} MUST NOT emit zero ops. LCS emits remove /x/0 + add /x/1 (Myers matches the equal empty containers at swapped indices, §5.5.2).",
  schema: null,
  original: { x: [[], {}] },
  modified: { x: [{}, []] },
});
...
// applyPatch({foo:1},[{op:"replace",path:"foo",value:2}]) returned 2 instead of rejecting. Fixed: §3.7
// requires INVALID_POINTER on all six ops and for both `path` and `from`, read-side included.
A("malformed-pointer", { name: "ptr-replace-no-leading-slash-invalid", comment: "§3.7 (D2): replace with path \"foo\" (no leading /) is a malformed whole-pointer -> INVALID_POINTER, NOT an alias to the root document", doc: { foo: 1 }, patch: [{ op: "replace", path: "foo", value: 2 } as unknown as Operation], error: { code: "INVALID_POINTER", index: 0 } });
A("malformed-pointer", { name: "ptr-add-no-leading-slash-invalid", comment: "§3.7 (D2): add with path \"foo\" (no leading /) -> INVALID_POINTER", doc: { foo: 1 }, patch: [{ op: "add", path: "foo", value: 2 } as unknown as Operation], error: { code: "INVALID_POINTER", index: 0 } });
...
A("test-required-value", { name: "test-missing-value-present-target-invalid", comment: "§8.3/§8.3.5 (D4): {op:test,path:/a} on {a:1} with NO `value` -> INVALID_OPERATION at tier 1, before any value comparison", doc: { a: 1 }, patch: [{ op: "test", path: "/a" } as unknown as Operation], error: { code: "INVALID_OPERATION", index: 0 } });
A("test-required-value", { name: "test-missing-value-absent-target-invalid", comment: "§8.3.5 (D4): missing `value` (tier 1) precedes read-side non-existence (tier 3) -> INVALID_OPERATION, not PATH_UNRESOLVABLE", doc: { a: 1 }, patch: [{ op: "test", path: "/missing" } as unknown as Operation], error: { code: "INVALID_OPERATION", index: 0 } });
