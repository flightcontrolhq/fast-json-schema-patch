...
import { createPatcher } from "../src/core/patcher";
...
function generatePerformanceCharts(metrics: BenchmarkMetrics[]) {
  ...
  const libraries = [
    "schema-json-patch (legacy)",
    "schema-json-patch (new)",
    "fast-json-patch",
    "jsondiffpatch",
  ];
  ...
  console.log("\n📊 Chart Legend:");
  console.log("🟢 Schema (Legacy) = schema-json-patch (legacy)");
  console.log("🔵 Schema (New) = schema-json-patch (new)");
...
function generateComprehensiveReport(allMetrics: BenchmarkMetrics[]) {
  ...
  const legacySchemaMetrics = byLibrary["schema-json-patch (legacy)"] || [];
  const newSchemaMetrics = byLibrary["schema-json-patch (new)"] || [];
...
async function compare() {
  ...
  for (const [name, { doc1, doc2, schema: scenarioSchema }] of Object.entries(
    ...
    await writeFile(
      join(__dirname, `${name}-legacy-schema-patch.json`),
      JSON.stringify(legacySchemaPatch, null, 2)
    );
    await writeFile(
    ...
    console.log(
      `  • schema-json-patch (legacy): ${legacySchemaPatch.length} operations`
    );
    console.log(
      `  • schema-json-patch (new): ${newSchemaPatch.operations.length} operations`
    );
  ...
  const legacyPatcher = new SchemaPatcher({ plan });
  const newPatcher = createPatcher(mainSchema as unknown as JSONSchema);
  ...
  for (const complexityRange of complexityRanges) {
    ...
    while (
      ...
      if (
        ...
        const libraries = [
          {
            name: "schema-json-patch (legacy)",
            fn: () => legacyPatcher.createPatch(doc1, doc2),
          },
          {
