...
export function createRandomCloudConfig() {
  const config: any = {
    environments: [],
  };

  const numEnvironments = faker.number.int({ min: 1, max: 2 });
  for (let i = 0; i < numEnvironments; i++) {
    const services = [];
    const numServices = faker.number.int({ min: 2, max: 20 });
    for (let j = 0; j < numServices; j++) {
      services.push({
        id: faker.lorem.slug(),
        name: faker.company.buzzPhrase(),
        type: faker.helpers.arrayElement(["web", "worker", "db"]),
        cpu: faker.helpers.arrayElement([0.25, 0.5, 1, 2, 4]),
        memory: faker.helpers.arrayElement([0.5, 1, 2, 4, 8]),
        minInstances: faker.number.int({ min: 1, max: 2 }),
        maxInstances: faker.number.int({ min: 2, max: 10 }),
      });
    }
    config.environments.push({
      ...
      source: {
        type: "github",
        owner: faker.company.name().toLowerCase(),
...
export const flightcontrolModifications = [
  ...
  },
  {
    ...
    modify: (doc: any) => {
      ...
      if (env && env.services.length > 0) {
        ...
        if (service)
          service.cpu = faker.helpers.arrayElement([0.125, 0.25, 0.5, 1, 2, 4]);
      }
    },
  },
  {
  ...
  },
  {
    ...
    modify: (doc: any) => {
      ...
      if (env && env.services.length > 0) {
        const service: any = faker.helpers.arrayElement(env.services);
        if (service) service.storage = faker.number.int({ min: 20, max: 200 });
      }
    },
  },
  {
  ...
  },
  {
    ...
    modify: (doc: any) => {
      ...
      if (env && env.services.length > 0) {
        const service: any = faker.helpers.arrayElement(env.services);
        if (service) service.port = faker.number.int({ min: 3000, max: 8080 });
      }
    },
  },
  {
  ...
  },
  {
    ...
    modify: (doc: any) => {
      ...
      if (env && env.services.length >= 2) {
        const [service1, service2]: any[] = faker.helpers.shuffle(env.services);
  ...
  },
  {
    ...
    modify: (doc: any) => {
      const env: any = faker.helpers.arrayElement(doc.environments);
      if (env && env.services.length > 0) {
        const rdsService: any = env.services.find((s: any) => s.type === "rds");
        if (rdsService) {
  ...
  },
  {
    ...
    modify: (doc: any) => {
      const env: any = faker.helpers.arrayElement(doc.environments);
      if (env && env.services.length > 0) {
        const webService: any = env.services.find((s: any) => s.type === "web");
        if (webService) {
...
export function applyModificationsForTargetComplexity(
  ...
  let appliedComplexity = 0;
  const appliedModifications: string[] = [];
  let availableModifications = [...flightcontrolModifications];

  const selectedForComplexity = selectModificationsForComplexity(
    availableModifications,
    targetComplexity,
    complexityRange
  );

  for (const modification of selectedForComplexity) {
    modification.modify(doc);
    appliedComplexity += modification.complexity;
    appliedModifications.push(modification.name);
  }

  return { appliedModifications, actualComplexity: appliedComplexity };
}
...
function selectModificationsForComplexity(
  ...
  let available = [...modifications];

  // Stratified sampling based on complexity range
  const filterByComplexity = (min: number, max: number) =>
    available.filter(
      (m) => m.complexity >= min && m.complexity <= max && !appliedNames.has(m.name)
    );

  let lowComplexityPool = filterByComplexity(1, 10);
  let mediumComplexityPool = filterByComplexity(11, 40);
  let highComplexityPool = filterByComplexity(41, 100);

  // Define strategy based on target complexity
  let strategy: { pool: any[]; count: number }[] = [];
  const fakerInt = (min: number, max: number) => faker.number.int({ min, max });

  if (complexityRange.label === "Small") {
    strategy = [
      { pool: lowComplexityPool, count: fakerInt(2, 5) },
      { pool: mediumComplexityPool, count: fakerInt(0, 1) },
    ];
  } else if (complexityRange.label === "Medium") {
    strategy = [
      { pool: lowComplexityPool, count: fakerInt(3, 6) },
      { pool: mediumComplexityPool, count: fakerInt(2, 4) },
      { pool: highComplexityPool, count: fakerInt(0, 1) },
    ];
  } else if (complexityRange.label === "Large") {
    strategy = [
      { pool: lowComplexityPool, count: fakerInt(2, 4) },
      { pool: mediumComplexityPool, count: fakerInt(3, 5) },
      { pool: highComplexityPool, count: fakerInt(2, 3) },
    ];
  } else {
    // Default strategy if range is not specified
    strategy = [{ pool: available, count: 5 }];
  }

  const pushRandom = (pool: any[]) => {
    if (pool.length > 0) {
      const selectedIndex = faker.number.int({ min: 0, max: pool.length - 1 });
      const mod = pool[selectedIndex];
      if (mod && !appliedNames.has(mod.name)) {
        selected.push(mod);
        appliedNames.add(mod.name);
        currentComplexity += mod.complexity;
        // Remove from all pools to avoid re-selection
        lowComplexityPool = lowComplexityPool.filter(m => m.name !== mod.name);
        mediumComplexityPool = mediumComplexityPool.filter(m => m.name !== mod.name);
        highComplexityPool = highComplexityPool.filter(m => m.name !== mod.name);
      }
    }
  };
  ...
  for (const rule of strategy) {
    for (let i = 0; i < rule.count; i++) {
      pushRandom(rule.pool);
    }
  }

  // Top-up if complexity is still too low
  while (currentComplexity < targetComplexity && available.length > 0) {
    const remainingComplexity = targetComplexity - currentComplexity;
    let suitablePool: any[];
    if (remainingComplexity > 40 && highComplexityPool.length > 0) {
      suitablePool = highComplexityPool;
    } else if (remainingComplexity > 10 && mediumComplexityPool.length > 0) {
      suitablePool = mediumComplexityPool;
    } else {
      suitablePool = lowComplexityPool;
    }

    if (suitablePool.length === 0) {
      // If the ideal pool is empty, try any other non-empty pool
      if (mediumComplexityPool.length > 0) suitablePool = mediumComplexityPool;
      else if (highComplexityPool.length > 0) suitablePool = highComplexityPool;
      else if (lowComplexityPool.length > 0) suitablePool = lowComplexityPool;
      else break; // No more modifications available
    }

    pushRandom(suitablePool);
    available = available.filter(m => !appliedNames.has(m.name));
  }
