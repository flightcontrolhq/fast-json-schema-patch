...
// member order and numeric literal text are preserved, CORE §1.2), derives a
...
// to the lcs strategy (GEN §3). When non-nil it is a JSON Schema whose array
// item shapes select per-array diff strategies — a `required` primary-key field,
// for example, auto-detects the primaryKey strategy (CORE §3.5). Plan derivation
...
// deep-equal. The result is deterministic for fixed inputs (CORE §1.3).
